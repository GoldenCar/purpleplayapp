<template>
    ...
    <q-toolbar style="background: #1A1A1A !important;">
        ...
        <q-btn id="closeVideoPlayer" :style="`padding: 0 .5rem; ${childLock ? 'opacity: .3;' : ''}`" @click="exitMedia()" flat>
        ...
        <q-toggle v-if="globalVideoLock" v-model="childLock" color="red" :icon="`${childLock ? 'fas fa-lock' : 'fas fa-unlock'}`" left-label label="<span class='gt-sm'>Child Lock</span>" style="margin-right: 16px;"/>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            chapterStyle() {
                return 'position: absolute; top: 0; width: 100%; z-index: 999;' + (this.brandColor ? 'background: ' + this.brandColor + ' !important;' : 'background: #777 !important;')
            },
        ...
        created() {
            if (this.globalVideoLock) this.childLock = true
        },
